...
def read_rfid():
    ...
    device = evdev.InputDevice('/dev/input/event0')  # Reemplaza X con el número correcto del dispositivo
    for event in device.read():
        if event.ev_type == evdev.ecodes.EV_KEY and event.value == 1:  # key down event
            key_code = evdev.ecodes.KEY[event.code]
            return key_code  # Retorna el código de la tecla que se presionó
